...
class IllinoisDOCDataset(Dataset):
    ...
    def __init__(self, root_dir, transform=None, max_samples_per_class=50, metadata_file=None):
        ...
        self.labels = []
        self.class_names = []
        self.criminal_status = []  # Additional field for criminal/non-criminal classification
        self.metadata = None
    ...
    def _load_dataset(self):
        ...
        # Illinois DOC dataset structure can vary:
        # Structure 1: root_dir/criminal_id/image_files
        # Structure 2: root_dir/images/ with separate labels file
        # Structure 3: root_dir/criminal/non_criminal/ subdirectories
    ...
    def _detect_dataset_structure(self):
        ...
        # Check for binary classification structure
        if set(subdirs) & {'criminal', 'non_criminal', 'offender', 'non_offender'}:
            return "binary_classification"
        
        # Check for hierarchical structure (multiple ID folders)
        if len(subdirs) > 2 and all(len(d) > 3 for d in subdirs[:5]):  # Assuming IDs are longer than 3 chars
            return "hierarchical"
    ...
    def _load_hierarchical_structure(self):
        ...
        for person_id in sorted(os.listdir(self.root_dir)):
            ...
            # Get criminal status from metadata if available
    ...
    def _load_binary_classification_structure(self):
        ...
        for category in categories:
            ...
            max_category_samples = self.max_samples_per_class * 10  # Allow more samples per category
            if len(image_files) > max_category_samples:
                image_files = np.random.choice(image_files, max_category_samples, replace=False)
            
            # Create pseudo-IDs for individual tracking
    ...
    def _load_flat_structure(self):
        ...
        # Extract person IDs from filenames (assuming format: personID_imageNum.jpg)
        person_images = {}
        for img_path in image_files:
            ...
            if '_' in filename:
                ...
                # Use first part of filename before any numbers
                import re
    ...
    def _get_criminal_status(self, person_id):
        ...
        if self.metadata is not None:
            # Try to match person_id with metadata
            person_data = self.metadata[self.metadata['id'].astype(str) == str(person_id)]
            if not person_data.empty:
                # Look for criminal status indicators in metadata columns
                status_columns = ['criminal', 'offender', 'status', 'category', 'label']
                for col in status_columns:
                    if col in person_data.columns:
                        status = person_data[col].iloc[0]
                        if status in ['criminal', 'offender', 1, True, 'yes']:
                            return 'criminal'
                        elif status in ['non_criminal', 'non_offender', 0, False, 'no']:
                            return 'non_criminal'
        
        # Infer from person_id or return unknown
...
class CriminalDetectionSystem:
    ...
    def __init__(self, illinois_doc_path, max_classes=100, max_samples_per_class=50, metadata_file=None):
        ...
        self.features_dict = {}
        self.results = {}
        self.binary_results = {}  # For criminal/non-criminal classification
    ...
    def train_traditional_models(self):
        ...
        # Train and evaluate models for identity classification
        ...
        # Criminal classification (if applicable)
    ...
    def train_deep_learning_models(self):
        ...
        # Train classifiers on extracted features for identity classification
    ...
    def evaluate_cross_validation(self, cv_folds=5):
        ...
        # Test key models with cross-validation
        models_to_test = {
    ...
    def visualize_results(self):
        ...
        # 4. Criminal Classification Results (if available)
    ...
    def _plot_confusion_matrix(self, model_name):
        ...
        # For readability, limit to top classes if too many
        if len(np.unique(y_test)) > 20:
            # Show only top 20 most frequent classes
...
def main():
    ...
    # Example configuration - Update these paths according to your setup
    illinois_doc_path = "/path/to/illinois_doc_dataset"  # Update this path
    metadata_file = "/path/to/metadata.csv"  # Optional metadata file
    
    # Initialize system
    detection_system = CriminalDetectionSystem(
        illinois_doc_path=illinois_doc_path,
        max_classes=50,  # Limit classes for demo
        max_samples_per_class=30,  # Limit samples per class
